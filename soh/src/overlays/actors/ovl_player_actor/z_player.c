...
// SoH: Multiplier applied to the roll animation's playback speed.
// Raising this makes the roll animation play faster AND finish sooner in real time.
// Since the per-frame movement speed in `Player_Action_Roll` is unaffected by this value,
// a faster roll simply has fewer real frames to move in, so it also travels proportionally
// less distance for the same input - this is what lets you spam the button for quick,
// continuous rolls (remake-style) instead of one long roll.
// Vanilla value is 1.0f (SoH shipped with 1.25f as a slight speedup).
#define PLAYER_ROLL_ANIM_SPEED_MULTIPLIER 2.2f

// SoH: Starting speed when holding the roll button transitions Link into running
// right after a roll (see `Player_Action_Roll`). Kept low ("not too fast") since the
// run's own acceleration logic will speed him up further the longer the stick/button is held.
// For reference, normal roll speed target is ~3.0-4.5f, and full run is much higher.
#define PLAYER_ROLL_HOLD_RUN_SPEED 4.0f
...
void Player_SetupRoll(Player* this, PlayState* play) {
    ...
    LinkAnimation_PlayOnceSetSpeed(play, &this->skelAnime,
                                   ...
                                   PLAYER_ROLL_ANIM_SPEED_MULTIPLIER * sWaterSpeedFactor);
        ...
        if (this->av2.bonked) {
            ...
            if ((this->skelAnime.curFrame < 15.0f) || !Player_ActionHandler_7(this, play)) {
                if (this->skelAnime.curFrame >= 20.0f) {
                    // SoH: If the roll button is still held when the roll finishes, transition
                    // into running (instead of stopping at idle). The run starts at a reduced
                    // speed (not the character's max run speed) and `func_8083C858` will pick
                    // the normal run animation for the player's current state on its own.
                    if (CHECK_BTN_ALL(sControlInput->cur.button, BTN_A)) {
                        this->linearVelocity = PLAYER_ROLL_HOLD_RUN_SPEED;
                        func_8083C858(this, play);
                    } else {
                        func_8083A060(this, play);
                    }
